...
build: check-go check-git
	...
	go build -o polygon-edge -ldflags="\
    	-X 'github.com/avenbreaks/raindrops/versioning.Version=$(VERSION)' \
		-X 'github.com/avenbreaks/raindrops/versioning.Commit=$(COMMIT_HASH)'\
		-X 'github.com/avenbreaks/raindrops/versioning.Branch=$(BRANCH)'\
		-X 'github.com/avenbreaks/raindrops/versioning.BuildTime=$(TIME)'" \
	main.go
